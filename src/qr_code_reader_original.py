...
def get_qr_coords(cmtx, dist, points, data):
...
def show_axes(cmtx, dist, img):
    ...
    if ret_qr:
        data = qr.decode(img, points)
        # axis point projects 3D coordinate points to a 2D plane
        axis_points = get_qr_coords(cmtx, dist, points, data[0])  # points	Output vector of vertices of the minimum-area quadrangle containing the code.
